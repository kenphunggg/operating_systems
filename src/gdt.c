...
void init_gdt()
{
    ...
    // Access flags (0x9A): Present(1) DPL(00) Type(1) Code(1) Conforming(0) Readable(1) Accessed(0)
    gdt_set_gate(1, 0, 0xFFFFFFFF, 0x9A, 0xCF);

    // 3. Kernel Data Segment (Ring 0).
    // Base=0, Limit=4GB, Access=Data(Read/Write), Granularity=4KB pages.
    // Access flags (0x92): Present(1) DPL(00) Type(1) Data(0) Direction(0) Writable(1) Accessed(0)
    ...
    asm volatile(
        ...
        "mov %%ax, %%fs\n\t"
        "mov %%ax, %%gs\n\t"
        "ljmp $0x08, $1f\n\t" // 0x08 is the offset of our code segment. Jump to a label '1'.
